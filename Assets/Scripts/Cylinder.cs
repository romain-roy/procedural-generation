...
public class Cylinder
{
    ...
    {
        nbVertices = (nbMeridians + 1 - nbCoupures * 2) * 4 + 2;
        ...
        for (int i = 0, j = 0; i <= nbMeridians; i++, j += 2)
        {
            ...
            uv[j + ((nbMeridians + 1 - nbCoupures * 2)) * 2] = new Vector2(uv[j].x, 1);
            uv[j + 1] = new Vector2(1f / nbMeridians * i, 0);
            uv[j + 1 + ((nbMeridians + 1 - nbCoupures * 2)) * 2] = new Vector2(uv[j].x, 1);

            if (i == indiceCoupure || i == nbMeridians - indiceCoupure - nbCoupures) i += nbCoupures;

            vertices[j] = new Vector3(x + size.x / 2, 0, y + size.y / 2);
            vertices[j + ((nbMeridians + 1 - nbCoupures * 2)) * 2] = new Vector3(x + size.x / 2, height, y + size.y / 2);
            vertices[j + 1] = vertices[j];
            vertices[j + 1 + ((nbMeridians + 1 - nbCoupures * 2)) * 2] = new Vector3(x + size.x / 2, height, y + size.y / 2);
        }
        ...
        for (int i = 0, k = 0; i < nbTrianglesCorps; i += 6, k += 2)
        {
            triangles[i] = triangles[i + 3] = k;
            triangles[i + 1] = k + ((nbMeridians + 1 - nbCoupures * 2)) * 2;
            triangles[i + 2] = triangles[i + 4] = k + ((nbMeridians - nbCoupures * 2) + 2) * 2;
            triangles[i + 5] = k + 2;
        }

        for (int i = 0, k = 0; k < (nbMeridians - nbCoupures * 2) * 2; i += 6, k += 2)
        {
            triangles[i + nbTrianglesCorps] = k + 1;
            triangles[i + nbTrianglesCorps + 1] = k + 3;
            triangles[i + nbTrianglesCorps + 2] = nbVertices - 2;
            triangles[i + nbTrianglesCorps + 3] = k + (nbMeridians + 1 - nbCoupures * 2) * 2 + 3;
            triangles[i + nbTrianglesCorps + 4] = k + (nbMeridians + 1 - nbCoupures * 2) * 2 + 1;
